...
uint256 hashGenesisBlock("0x000006a5d9160b0f85822870b1871061b5bab42afb0260843dc0a999cd4219d6");
...
const string strMessageMagic = "UltimateBitcoin Signed Message:\n";
...
{
    // UltimateBitcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // UltimateBitcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "ulb genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	   ...
	   // assert(block.hashMerkleRoot == uint256("0x00000097ed76cd5f08b75d118fb7c99aec7cdd297b96c21dc6671d2034c953"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd4, 0xc3, 0xaf, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("UltimateBitcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("UltimateBitcoinMiner : ProcessBlock, block not accepted");
    }
...
void static UltimateBitcoinMiner(CWallet *pwallet)
{
    printf("UltimateBitcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the mining thread
    RenameThread("UltimateBitcoin-miner");
    ...
    unsigned int nExtraNonce = 0;
        ...
        printf("Running UltimateBitcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&UltimateBitcoinMiner, pwallet));
}
